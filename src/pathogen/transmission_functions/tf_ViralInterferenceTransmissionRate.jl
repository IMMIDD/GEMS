...
factor. The interference then persists after recovery, waning linearly from the full
factor back to 1.0 over a per-pathogen `persistence` window (in ticks); only natural
recovery counts, so vaccine-derived immunity never triggers interference.
...
no interference). Persistence windows are given the same way via `persistence`, falling
back to `default_persistence` (default: 0.0, i.e. no post-recovery interference).
...
- `persistence::Dict{String,Float64}`: Maps a pathogen's name to the post-recovery
  window (ticks, ≥ 0) over which its interference wanes to zero.
- `default_persistence::Float64`: Fallback window for pathogens not listed in
  `persistence` (≥ 0). Defaults to 0.0 (no post-recovery interference).

# Example

```julia
# An active Influenza infection reduces susceptibility to the exposed pathogen by 60%;
# after RSV recovery, its reduction lingers and wanes over 14 ticks.
modifier = ViralInterferenceModifier(
    interferences = [("Influenza", 0.4), ("RSV", 0.6)],
    default_interference_factor = 1.0,
    persistence = [("RSV", 14.0)]
)
...
struct ViralInterferenceModifier <: TransmissionModifier
    ...
    persistence::Dict{String, Float64}
    default_persistence::Float64

    function ViralInterferenceModifier(;
            interferences = Tuple{String, Float64}[],
            default_interference_factor::Float64 = 1.0,
            persistence = Tuple{String, Float64}[],
            default_persistence::Float64 = 0.0)
        ...
        default_persistence < 0.0 &&
            throw(ArgumentError("default_persistence must be non-negative."))
        ...
        # accepts a list of (name, ticks) pairs, incl. TOML's [name, ticks] vectors
        windows = Dict{String, Float64}()
        for entry in persistence
            name = String(entry[1])
            ticks = Float64(entry[2])
            isempty(name) &&
                throw(ArgumentError("persistence pathogen names must be non-empty."))
            ticks < 0.0 &&
                throw(ArgumentError("persistence durations must be non-negative."))
            haskey(windows, name) &&
                throw(ArgumentError("Duplicate pathogen name '$name' in persistence."))
            windows[name] = ticks
        end

        return new(factors, default_interference_factor, windows, default_persistence)
    end
end
...
Base.show(io::IO, m::ViralInterferenceModifier) = print(io,
    ...
    "default_persistence=$(m.default_persistence))")
...
1.0 means no interference, 0.0 means complete suppression. Combines the full interference
of each concurrent active infection with the linearly-waning residual interference of each
pathogen the infectee recovered from within its persistence window.
...
function transmission_factor(
    ...
    # during-infection interference: full factor while a concurrent infection is active
    ...
    # post-recovery interference: linearly-waning residual within the persistence window.
    for s in each_immunity(infectee, sim)
        s.pathogen_id == pathogen_id && continue
        natural_immunity_active(s, tick) || continue
        name = get_pathogen(sim, s.pathogen_id).name
        window = get(modifier.persistence, name, modifier.default_persistence)
        window <= 0.0 && continue
        elapsed = Float64(tick - s.natural_acquired_tick)
        elapsed >= window && continue
        f = get(modifier.interferences, name, modifier.default_interference_factor)
        remaining_susceptibility *= f + (1.0 - f) * (elapsed / window)
    end
...
"""
    transmission_factor_bound(modifier::ViralInterferenceModifier, pathogen_id::Int8, infecter::Individual, setting::Setting, tick::Int16, sim::Simulation)::Float64

Returns `1.0`, the factor of an infectee without interference.
"""
transmission_factor_bound(modifier::ViralInterferenceModifier, pathogen_id::Int8, infecter::Individual, setting::Setting, tick::Int16, sim::Simulation)::Float64 = 1.0
...
Base.show(io::IO, tf::ViralInterferenceTransmissionRate) = print(io,
    ...
    "persistence=$(tf.modifier.persistence), " *
    "default_persistence=$(tf.modifier.default_persistence))")
...
"""
    transmission_bound(transFunc::ViralInterferenceTransmissionRate, pathogen_id::Int8, infecter::Individual, setting::Setting, tick::Int16, sim::Simulation)::Float64
