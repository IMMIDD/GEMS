...
function calculate_progression(individual::Individual, tick::Int16, dp::Critical, rng::Xoshiro)
    ...
    infectiousness_onset::Int16 = rand_round(tick + 1 + _rand_val(dp.exposure_to_infectiousness_onset, rng))

    # Calculate the time to symptom onset
    symptom_onset::Int16 = rand_round(infectiousness_onset + _rand_val(dp.infectiousness_onset_to_symptom_onset, rng))

    # Calculate the time to severeness onset
    severeness_onset::Int16 = rand_round(symptom_onset + _rand_val(dp.symptom_onset_to_severeness_onset, rng))

    # Calculate the time to critical onset
    critical_onset::Int16 = rand_round(severeness_onset + _rand_val(dp.severeness_onset_to_critical_onset, rng))

    # Calculate the time to critical offset
    critical_offset::Int16 = rand_round(critical_onset + _rand_val(dp.critical_onset_to_critical_offset, rng))

    # Calculate the time to severeness offset
    severeness_offset::Int16 = rand_round(critical_offset + _rand_val(dp.critical_offset_to_severeness_offset, rng))
