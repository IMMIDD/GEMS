...
function calculate_progression(individual::Individual, tick::Int16, dp::Critical, rng::Xoshiro)
    ...
    infectiousness_onset::Int16 = Int16(tick + 1 + rand_val(dp.exposure_to_infectiousness_onset, rng))

    # Calculate the time to symptom onset
    symptom_onset::Int16 = Int16(infectiousness_onset + rand_val(dp.infectiousness_onset_to_symptom_onset, rng))

    # Calculate the time to severeness onset
    severeness_onset::Int16 = Int16(symptom_onset + rand_val(dp.symptom_onset_to_severeness_onset, rng))

    # Calculate the time to hospital admission
    hospital_admission::Int16 = Int16(severeness_onset + rand_val(dp.severeness_onset_to_hospital_admission, rng))

    # Calculate the time to ICU admission
    icu_admission::Int16 = Int16(hospital_admission + rand_val(dp.hospital_admission_to_icu_admission, rng))
    ...
    icu_discharge::Int16 = Int16(icu_admission + rand_val(dp.icu_admission_to_icu_discharge, rng))

    # Calculate the time to hospital discharge
    hospital_discharge::Int16 = Int16(icu_discharge + rand_val(dp.icu_discharge_to_hospital_discharge, rng))

    # Calculate the time to severeness offset
    severeness_offset::Int16 = Int16(hospital_discharge + rand_val(dp.hospital_discharge_to_severeness_offset, rng))

    # Calculate the time to recovery
    recovery::Int16 = Int16(severeness_offset + rand_val(dp.severeness_offset_to_recovery, rng))
