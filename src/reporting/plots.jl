...
|`:AggregatedSettingAgeContacts`   | Age-X-Age contact matrix for all setting types in the simulation (e.g., `Household`). | No         | `settingtype::DataType`: Filter for specific setting type (e.g., `Household`) |
