...
function _incidence_subplot(incidence, utick; plotargs...)
    ttl = "7-$utick Incidence per 100,000 Individuals"
    groupedbar([incidence[!, "a0_10"] incidence[!, "a11_20"] incidence[!, "a21_30"] incidence[!, "a31_40"] incidence[!, "a41_50"] incidence[!, "a51_60"] incidence[!, "a61_70"] incidence[!, "a71_80"] incidence[!, "a81_90"] incidence[!, "a91_100"]],
        bar_position = :stack,
        bar_width = 0.6,
        xlabel = utick,
        ylabel = ttl,
...
"""
function generate(plt::Incidence, rd::ResultData; pathogen = nothing, plotargs...)
    ...
    all_incidence, pids, pnames, _ = _pathogen_setup(age_incidence(rd), rd, pathogen)

    subplots = [_incidence_subplot(
        filter(row -> row.pathogen_id == pid, all_incidence), utick;
