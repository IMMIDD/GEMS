...
function _observed_R_subplot(data, ft, uticks; plotargs...)
    upper_ticks = uppercasefirst(uticks)
    y_max = isempty(data) ? 2 : maximum(data.upper_est_R) + 1
    p = plot(xlabel = upper_ticks * "s", ylabel = "Observed Effective R",
        xlim = (0, ft), ylim = (0, y_max), dpi = 300, fontfamily = "Times Roman")
    if !isempty(data)
        plot!(p, data[!, "tick"], data[!, "mean_est_R"], label="Mean Estimated R", linewidth=2, color="blue")
        plot!(p, data[!, "tick"], data[!, "lower_est_R"], fillrange=data[!, "upper_est_R"],
            label = "95% Confidence Band", alpha=0.2, color="blue")
        plot!(p, data[!, "tick"], data[!, "lower_est_R"], linewidth=1, linestyle=:dot, label=nothing, alpha=0.5, color="blue")
        plot!(p, data[!, "tick"], data[!, "upper_est_R"], linewidth=1, linestyle=:dot, label=nothing, alpha=0.5, color="blue")
...
"""
function generate(plt::ObservedReproduction, rd::ResultData; pathogen = nothing, plotargs...)
    ...
    or_all, pids, pnames, _ = _pathogen_setup(observed_R(rd), rd, pathogen)

    subplots = [_observed_R_subplot(
        filter(row -> row.pathogen_id == pid, or_all) |> dropmissing, ft, uticks;
