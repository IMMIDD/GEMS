...
function _time_to_detection_subplot(detection_times, uticks; plotargs...)
    ...
    p = plot(detection_times.tick, detection_times.mean_time_to_detection,
        label = "Mean Time To Detection",
        xlabel = upper_ticks * "s",
        ylabel = "Time To Detection in $(upper_ticks)s",
        fontfamily = "Times Roman", dpi = 300)
    vspan!(p, emptyzones, linewidth = 0, fillcolor = :red, alpha = 0.3, label = "No Detected Cases")
    plot!(p; plotargs...)
    return p
end
...
"""
function generate(plt::TimeToDetection, rd::ResultData; pathogen = nothing, plotargs...)
    ...
    desc *= "Red shaded areas indicate that no infection was detected during that period."
    description!(plt, desc)

    ttd_all, pids, pnames, _ = _pathogen_setup(time_to_detection(rd), rd, pathogen)

    subplots = [_time_to_detection_subplot(
        filter(row -> row.pathogen_id == pid, ttd_all), uticks;
