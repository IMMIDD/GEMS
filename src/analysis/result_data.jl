...
export tests, tick_pooltests, detected_tick_cases, rolling_observed_SI, time_to_detection, detection_rate, cumulative_quarantines, tick_hosptitalizations, health_episodes, tick_serial_intervals
...
function process_funcs(func_dicts::Dict)
    ...
    if POST_PROCESSING_PARALLELISM == :all && Sys.free_memory() / Sys.total_memory() < 0.25
        @warn "Post processing runs with POST_PROCESSING_PARALLELISM = :all and less than 25% of the system memory is free. The memory-heavy steps run at the same time in this mode; set the flag to :builtin in constants.jl if the system runs out of memory."
    end
    ...
    isempty(tasks) || print("\r$(_subinfo("$(length(tasks)) concurrent steps"))")
    for t in tasks
...
# `:builtin` only runs GEMS' own functions, as a style of the user's own may not be safe in parallel
...
dir = _basefolder() * "/src/analysis/rd_styles"
...
"""
    health_episodes(rd::ResultData)

Returns the host-level care episodes DataFrame (hospital/ICU/ventilation stays).
Look up the `PostProcessor` docs to find the column definitions.
Returns an empty dictionary if the data is not available in the input `ResultData` object.
"""
function health_episodes(rd::ResultData)
    return(get(rd |> dataframes, "health_episodes", Dict()))
end
